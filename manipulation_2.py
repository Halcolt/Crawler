...
def print_top_five_drive():
    ...
    if (option == "Y" or option == "y"):
        results = execute_query(mycursor, "SELECT DISTINCT MOption FROM MoreDriveOption")

        print("Here are all the additional options we have in our database:\n")
        for result in results:
            print("+ ", result[0])

        Moreoption = input("Enter the Option you want: ")
        params = (People, Money, Capacity, Moreoption)
    else:
        params = (People, Money, Capacity)

    query = """
        SELECT ProviderDriveOption.ProverderOpID, ProviderDriveOption.ProviderName, ProviderDriveOption.Price, ProviderDriveOption.Capacity,
        AVG(((PricePoint + PeoplePoint)/2 + GetAppPoint)/2) as average
        ...
        """.format("and MOption = %s" if option == "Y" or option == "y" else "")
    result = execute_query(mycursor, query, params)

    if not result:
        print("There are no suitable providers for you now, please Re-Enter the value you want for Cloud option")
        mycursor.close()
        return

    result1 = execute_query(mycursor, "SELECT COUNT(ProviderName) AS NumberOfProviders FROM ProType WHERE PType = 'Drive'")
    result2 = execute_query(mycursor, "SELECT COUNT(ProverderOpID) AS NumberOfProviderOPs FROM ProviderDriveOption")

    print("From {} providers from our database with a total of {} options, we give you the top-ranking choices based on your requirements".format(result1[0][0], result2[0][0]))

    update_list = []
    for row in result:
        update_list.append((row[4], row[0], row[1], row[2], row[3]))
